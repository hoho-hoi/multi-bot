...
- States: [ `STATE_REQUIREMENT_PR_OPEN` -> `STATE_MANAGER_REVIEW_IN_PROGRESS` -> `STATE_REQUIREMENT_CHANGES_REQUESTED` -> `STATE_REQUIREMENT_APPROVED` -> `STATE_REQUIREMENT_PR_CLOSED` -> `STATE_MILESTONE_PLANNING` -> `STATE_IMPLEMENTATION_BACKLOG_READY` -> `STATE_IMPLEMENTATION_REVIEW_IN_PROGRESS` -> `STATE_DELIVERY_COMPLETED` ]
...
- States: [ `STATE_IMPLEMENTATION_BACKLOG_READY` -> `STATE_ENGINEER_JOB_RUNNING` -> `STATE_TEST_FIX_IN_PROGRESS` -> `STATE_IMPLEMENTATION_PR_OPEN` -> `STATE_ENGINEER_CHANGES_REQUESTED` -> `STATE_IMPLEMENTATION_PR_APPROVED` -> `STATE_IMPLEMENTATION_PR_MERGED` ]
